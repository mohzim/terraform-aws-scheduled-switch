...
init_command              = "terraform -chdir=terraform init -backend-config=\"bucket=mohzim-terraform\" -backend-config=\"key=terraform-aws-scheduled-switch-tfstate\" -backend-config=\"region=ap-south-1\" -input=false" # Replace all fields of REPLACE_ME with your s3 backend configuration values that is used to deploy the MWAA environment.
kill_command              = "terraform -chdir=terraform apply -input=false -target=aws_mwaa_environment.this -var enabled=false -auto-approve"
revive_command            = "terraform -chdir=terraform apply -input=false -target=aws_mwaa_environment.this -var enabled=true -auto-approve"
